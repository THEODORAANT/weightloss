...
class PerchShop_Cart extends PerchShop_Factory
{
	...
	public function __construct($api=false)
	{
		...
		public function init_api($cart_id,$memberID)
    	{
    	$sql = 'SELECT * FROM '.$this->table.' WHERE memberID='.$this->db->pdb((int)$memberID).' and cartID='.$this->db->pdb((int)$cart_id);
        		$row = $this->db->get_row($sql);

         if (PerchUtil::count($row)) {

        $this->cartID =$cart_id;
         $this->cartPricing= 'standard';
        return $this;
        }
        return false;
    	}
...
//echo "memberID";echo $member_id;
		...
		if ($replace) {
			$sql = 'DELETE FROM '.$this->item_table.' WHERE cartID='.$this->db->pdb($cart_id).' AND productID='.$this->db->pdb((int)$product);
			$this->db->execute($sql);
		}else{
			// already in the cart?
			$sql = 'SELECT itemID, itemQty FROM '.$this->item_table.' WHERE cartID='.$this->db->pdb((int)$cart_id).' AND productID='.$this->db->pdb((int)$product);
			$row = $this->db->get_row($sql);
			if (PerchUtil::count($row)) {
				$existing_id = (int)$row['itemID'];
				$existing_qty = (int)$row['itemQty'];
		...
		if ($existing_id) {
            ...
            //$data=$this->calculate_cart_for_api($cart_id);
...
public function calculate_cart_for_api($cart_id){
...
if (PerchUtil::count($cart) && isset($cart['locationID'])) {
		...
		// item counts
		$sql = 'SELECT COUNT(productID) AS product_count, SUM(itemQty) AS item_count FROM '.PERCH_DB_PREFIX.'shop_cart_items WHERE cartID='.(int)$cart_id;
		$row = $this->db->get_row($sql);
		if (is_array($row)) {
		...
		// Items
		$items = $Items->get_for_cart((int)$cart_id);
